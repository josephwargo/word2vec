...
class word2vec(object):
    # constructor
    def __init__ (self, corpus, features, window, stepSize=.025, numNegSamples=5):
        """
        Constructor
        """
        ...
        self.corpusWords = list(set(self.allCorpusWords))
        ...
        self.allContext = np.zeros((len(self.corpusWords),features))
        # self.allCenter = np.random.normal(0, .001, (len(self.corpusWords),features))
        self.allCenter = np.random.uniform(0, 1, size=(len(self.corpusWords),features))
